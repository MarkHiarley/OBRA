...
// GetDiariosConsolidadosByObra busca diários da view consolidada por obra
func (pr *DiarioServices) GetDiariosConsolidadosByObra(obraID int64) ([]models.DiarioConsolidado, error) {
	query := `
		...
		WHERE obra_id = $1
		ORDER BY data ASC, periodo
	`

	rows, err := pr.connection.Query(query, obraID)
	if err != nil {
		return nil, fmt.Errorf("erro ao buscar diários consolidados: %v", err)
	}
	defer rows.Close()
